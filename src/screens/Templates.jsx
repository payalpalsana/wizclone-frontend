  ...
  return (
    ...
    >
      <button
        ...
        {...listeners}
        type="button"
        style={{ cursor: 'grab', color: 'var(--text-muted)', border: 'none', background: 'none', padding: '0 2px', display: 'flex', flexShrink: 0 }}
      >
      ...
      <input
        ...
        style={{
          flex: 1, height: 34, fontSize: 14, borderRadius: 7,
          border: '1px solid var(--border)', backgroundColor: 'var(--bg-primary)',
          color: 'var(--text-primary)', padding: '0 10px', outline: 'none',
          fontFamily: 'inherit', transition: 'border-color 120ms, box-shadow 120ms',
        }}
      ...
      <button
        ...
        style={{ color: 'var(--text-muted)', border: 'none', background: 'none', cursor: 'pointer', padding: '0 2px', display: 'flex', flexShrink: 0, transition: 'opacity 120ms' }}
      >
        <IconX size={14} />
      </button>
    </div>
...
// --- Inline editor panel (expands below card) ---
function TemplateEditor({ template, onSave, onCancel }) {
  ...
  const editSub = (id, val) => setSubitems((prev) => prev.map((s) => s.id === id ? { ...s, name: val } : s))
  const deleteSub = (id) => setSubitems((prev) => prev.filter((s) => s.id !== id))
  const addSub = () => setSubitems((prev) => [...prev, { id: ++_itemId, name: '' }])
  ...
  const handleSave = async () => {
    ...
    onSave({ ...template, name: name.trim(), subitems: subitems.filter((s) => s.name.trim()) })
    setSaving(false)
  }
      ...
      >
        ...
        <div className="mb-4">
          ...
          <input
            ...
            style={{
              width: '100%', height: 36, fontSize: 14, borderRadius: 8,
              border: '1px solid var(--border)', backgroundColor: 'var(--bg-primary)',
              color: 'var(--text-primary)', padding: '0 12px', outline: 'none',
              fontFamily: 'inherit', transition: 'border-color 120ms, box-shadow 120ms',
              fontWeight: 500,
            }}
            onFocus={(e) => { e.target.style.borderColor = 'var(--accent)'; e.target.style.boxShadow = '0 0 0 3px rgba(108,71,255,0.12)' }}
        ...
        {/* Actions */}
        <div className="flex items-center gap-2 justify-end" style={{ paddingTop: 12, borderTop: '1px solid var(--border)' }}>
          <button
            type="button"
            onClick={onCancel}
            style={{
              height: 34, paddingInline: 14, borderRadius: 8, border: '1px solid var(--border)',
              backgroundColor: 'transparent', color: 'var(--text-primary)', fontSize: 14,
          ...
          <button
            ...
            style={{
              ...
              display: 'flex', alignItems: 'center', gap: 6, opacity: !name.trim() ? 0.6 : 1,
            }}
          >
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <button
          ...
          style={{ color: 'var(--text-muted)', border: 'none', background: 'none', cursor: 'pointer', padding: 0, display: 'flex', flexShrink: 0 }}
        >
          {expanded && !editing
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-xs" style={{ color: 'var(--text-muted)' }}>
            {template.subitems.length} subitem{template.subitems.length !== 1 ? 's' : ''} · Created {relDate(template.createdAt)}
          </span>
        </div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {!confirmDelete ? (
            ...
            <div className="flex items-center gap-1">
              <span className="text-xs" style={{ color: 'var(--danger)', whiteSpace: 'nowrap' }}>Delete?</span>
              <button
                type="button"
      ...
      <AnimatePresence>
        ...
        {expanded && !editing && (
          ...
          >
            <ul
              style={{
                margin: 0, padding: '12px 16px 14px 44px',
            ...
            >
              {template.subitems.map((s, i) => (
                ...
                >
                  <span style={{ color: 'var(--text-muted)', fontSize: 12, width: 18, textAlign: 'right', flexShrink: 0 }}>
                    {i + 1}.
                  </span>
                  {s.name}
                </li>
        ...
        <EmptyState
          illustration={
            <svg width="56" height="56" viewBox="0 0 56 56" fill="none">
              <rect x="10" y="12" width="36" height="32" rx="5" fill="var(--bg-secondary)" stroke="var(--border)" />
              <rect x="17" y="20" width="22" height="3" rx="1.5" fill="var(--skeleton)" />
              <rect x="17" y="27" width="16" height="3" rx="1.5" fill="var(--skeleton)" />
              <rect x="17" y="34" width="19" height="3" rx="1.5" fill="var(--skeleton)" />
              <circle cx="43" cy="14" r="7" fill="var(--accent-light)" />
              <path d="M43 11v3m0 3v.5M43 14h.01" stroke="var(--accent)" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          }
