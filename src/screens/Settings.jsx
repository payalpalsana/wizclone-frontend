...
export default function Settings() {
  ...
  // Sidebar width to offset sticky bar
      ...
      <div className="flex flex-col gap-4 flex-1">
        ...
        {boardsLoading ? (
          ...
          <Card>
            ...
            <p className="text-sm mb-3" style={{ color: 'var(--text-secondary)', margin: '0 0 12px' }}>
            ...
            <div
              ...
              style={{
                ...
                border: templateBoardId ? '1px solid var(--border)' : '1px dashed var(--border)',
                marginTop: 12,
              }}
            >
              {!templateBoardId ? (
                ...
                <div className="flex flex-col gap-2">
                  {[1, 2, 3].map((i) => (
                    <div key={i} className="skeleton-shimmer rounded" style={{ height: 14, width: i === 1 ? '60%' : i === 2 ? '80%' : '50%', borderRadius: 4 }} />
                  ))}
                </div>
              ) : templateItems.length === 0 ? (
                <p className="text-sm" style={{ color: 'var(--text-muted)' }}>No items found in this board.</p>
              ) : (
                <ul style={{ margin: 0, padding: 0, listStyle: 'none' }}>
                      ...
                      {item.subitems?.length > 0 && (
                        <ul style={{ margin: '2px 0 0 16px', padding: 0, listStyle: 'none' }}>
                          {item.subitems.map((sub) => (
                            <li key={sub.id} className="text-xs" style={{ color: 'var(--text-secondary)', lineHeight: 1.8 }}>• {sub.name}</li>
                          ))}
                        </ul>
                      )}
                    </li>
                  ...
                  {templateItems.length > 5 && (
                    <li className="text-xs" style={{ color: 'var(--accent)', cursor: 'pointer', marginTop: 4 }}>
        ...
        <Card>
          ...
          <p className="text-sm mb-4" style={{ color: 'var(--text-secondary)', margin: '0 0 16px' }}>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: isMobile ? 1 : undefined }}>
                    <div className="font-medium text-sm" style={{ color: 'var(--text-primary)', marginBottom: isMobile ? 0 : 2 }}>
                      {opt.label}
                      {opt.recommended && isMobile && (
                        <span style={{ marginLeft: 6, backgroundColor: 'var(--accent)', color: '#fff', fontSize: 9, fontWeight: 600, padding: '1px 5px', borderRadius: 3 }}>
                          Recommended
                        </span>
                      )}
                    </div>
                    {!isMobile && (
                      <>
                        <div className="text-xs mb-1" style={{ color: 'var(--text-muted)' }}>{opt.threshold}</div>
                        <div className="text-xs" style={{ color: 'var(--text-secondary)', lineHeight: 1.4 }}>{opt.description}</div>
                      </>
                    )}
                  </div>
                  {isMobile && (
                    <span className="text-xs" style={{ color: 'var(--text-muted)', flexShrink: 0 }}>{opt.threshold}</span>
                  )}
                </button>
              )
            })}
        ...
        <Card>
          ...
          {!automationEnabled && (
            <div
              ...
              style={{ backgroundColor: 'var(--warning-light)', color: 'var(--warning)', marginTop: 12 }}
            >
              Automation is paused. New items will not receive subitems automatically.
            </div>
          )}
        </Card>
      </div>
      ...
      >
        <button
          ...
          style={{
            ...
            fontFamily: 'inherit', opacity: saveState === 'saving' ? 0.8 : 1,
          }}
        >
          {saveState === 'saving' && <IconLoader2 size={14} className="animate-spin" />}
